...
async function category(tid, pg, filter, extend) {
    ...
    let videos = _.map(items, (item) => {
        ...
        return {
            vod_id: it.attribs.href.replace(/.*?\/index.php\/vod\/play\/id\/(.*)\/sid\/1\/nid\/1/g, '$1'),
...
async function detail(id) {
    ...
    const playlist = ['点击播放' + '$' + HOST + '/index.php/vod/play/id/' + id + '/sid/1/nid/1'];
...
async function play(flag, id, flags) {
    const link = HOST + '/index.php/vod/play/id/' + id + '/sid/1/nid/1';
    const html = await request(link);
    const $ = load(html);
